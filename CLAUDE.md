...
- **Windows OCR language packs:** with a source language picked, the page passes `lang` to `ocr`; if no installed recognizer matches it, the script returns `{ missing }` and the page asks with `tiny.dialog.confirm` to install the pack. `installOcrLanguage` runs `Add-WindowsCapability Language.OCR~~~<tag>~0.0.1.0` elevated (`INSTALL_OCR_PS1`, UAC) and returns ok / cancelled / failed. `OCR_PACKS` in `main.js` maps app codes to pack tags; he / uk / fa / hi have none. On failure the page offers `ms-settings:regionlanguage`. `tiny.macos.ocr` takes no language, so none of this applies on macOS.
- **Screen region (second hotkey, default Ctrl/Cmd+Shift+O, stored as `ocrHotkey`):** `captureRegion` in `main.js` hides the window. Windows: `captureScreen` of the monitor under the mouse, then our own frameless full-screen always-on-top window `snip.html` shows it frozen; the user drags a rectangle (Esc / right-click cancels) and it reports it normalized via `snipDone`. The OCR script crops + upscales with System.Drawing (`-Crop x,y,w,h`). Needs `"readAccess": true` so the page can load the temp screenshot. No Snipping Tool involved. macOS: `screencapture -i -x <tmp.png>`. Either way the backend pushes `translate-image` ({ path, rect? }) and the page runs `translateImage`.
